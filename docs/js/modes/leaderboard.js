 ...
 * @updated     2026-06-24
 * @changelog
 *   1.0.0 — Leaderboard browse screen. Tabs for gen1/gen2 × single/victoryroad/
 *           safari. Loads top 10 per board from Firebase. Shows "your rank" if
 *           the player has a score on that board. Live refresh button.
 */
...
export function createLeaderboard({ mount, config, data, params, onExit }) {
  ...
  let tab = 0; // index into BOARDS
  let uid = null;
  getIdentity().then((id) => { uid = id.uid; }).catch(() => {});
  ...
  function render() {
    clear(root).append(
      ...
      el('div', { class: 'lb-board', id: 'lb-board' }, loadingEl()),
  ...
  async function loadBoard() {
    ...
    const { gen, mode } = BOARDS[tab];
    try {
      const entries = await topEntries(gen, mode, 10);
      if (!board) return;
      if (!entries.length) {
        board.replaceChildren(el('div', { class: 'lb-empty' }, 'No scores yet — be the first!'));
        return;
      }
      board.replaceChildren(
        el('table', { class: 'lb-table' },
          el('thead', {}, el('tr', {},
            el('th', {}, '#'), el('th', {}, 'Player'), el('th', {}, 'Score'), el('th', {}, 'Detail'))),
          el('tbody', {},
            ...entries.map((e, i) => {
              const isMe = uid && e.uid === uid;
              return el('tr', { class: isMe ? 'lb-me' : '' },
                el('td', {}, PLACE[i] || String(i + 1)),
                el('td', { style: { fontWeight: isMe ? 800 : 400, color: isMe ? 'var(--accent-gold)' : '' } }, e.name || 'Anonymous'),
                el('td', { style: { fontWeight: 700 } }, String(e.score)),
                el('td', { class: 'lb-detail' }, e.detail || ''));
            }))));
    } catch {
      if (board) board.replaceChildren(el('div', { class: 'lb-empty' }, 'Could not load scores — check your connection.'));
    }
  }

  function loadingEl() { return el('div', { class: 'lb-empty' }, 'Loading\u2026'); }
  const PLACE = ['\uD83E\uDD47', '\uD83E\uDD48', '\uD83E\uDD49'];
